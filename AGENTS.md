...
- **`Wait+Screen` matches the whole screen.** Wait only for a marker unique to the new state, never for text still visible from an earlier step or contained in the command just typed. Clear the screen or press `Ctrl+L` between repeated prompts and use short sleeps inside a cycle.
